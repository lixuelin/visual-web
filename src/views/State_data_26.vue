    ...
    <div class="apollo-state-laybox" v-show="isDanger">
      ...
      <div class="apollo-state-laybox-body">
        <div class="apollo-state-laybox-car">
          <span v-if="dangerCarPart===1"></span>
          <span v-if="dangerCarPart===3" class="apollo-state-laybox-car-ivi"></span>
          <span v-if="dangerCarPart===6" class="apollo-state-laybox-car-dashboard"></span>
          <span v-if="dangerCarPart===8" class="apollo-state-laybox-car-camera"></span>
          <span v-if="dangerCarPart===2" class="apollo-state-laybox-car-ecu"></span>
          <span v-if="dangerCarPart===7" class="apollo-state-laybox-car-lidar"></span>
          <span v-if="dangerCarPart===9" class="apollo-state-laybox-car-gps"></span>
          <span v-if="dangerCarPart===5" class="apollo-state-laybox-car-canBus"></span>
          <span v-if="dangerCarPart===4" class="apollo-state-laybox-car-tbox"></span>
          <span v-if="dangerCarPart===10" class="apollo-state-laybox-car-carCount"></span>
        </div>
        <div class="apollo-state-laybox-cont" v-for="item in dangerCar">
          <div>
            <h4>CAR’S INFORMATION</h4>
            <p>车辆Id：{{item.uuid}}</p>
            <p>位置：{{item.country}}.{{item.continent}}.{{item.province}}</p>
            <p>攻击等级：{{item.attack_level}}</p>
            <p>攻击部位名称：{{item.part_name}}</p>
          </div>
          <div>
            <h4>HACKER’S INFORMATION</h4>
            <p>Ip：{{item.hacker_ip}}</p>
            <p>位置：{{item.hacker_country}}.{{item.hacker_province}}</p>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      repeatMe: function () {
        this.getLineData()
        this.timerInner = setTimeout(this.repeatMe, 20000)
      },
      intervalGet: function () {
        if (this.$route.name === 'State') {
          this.timer = setTimeout(this.repeatMe, 20000)
        } else {
      ...
      getDanger: function () {
        ...
        this.lineData.forEach((item, i) => {
          setTimeout(() => {
            this.setData(convertData, onlineData, item, i)
            setTimeout(() => {
              this.clearData()
            }, 4000)
          }, 5000 * i)
      ...
      setData: function (convertData, onlineData,item, i) {
        let series = []
        let data = item[1]
        series.push({
          type: 'lines',
