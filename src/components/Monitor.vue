  ...
  <div>
    <div  class="apollo-vehicle-search">
      <div class="apollo-vehicle-search-set">
        <button type="button" class="apollo-btn apollo-btn-default" :class="{'apollo-btn-disabled': dataCheckLen === 0}" @click="cancel" :disabled="dataCheckLen===0">{{this.language.top[0]}}</button>
    ...
    <div>
      ...
      <page-view :page="pageData" ref="resetTable" @pageOpt="pageOpt"></page-view>
    </div>
    <div class="alert" v-show="addcomment">
      <div class="left"><img src="../../static/img/OK.png" alt=""></div>
      <div class="right">{{this.language.success[0]}}</div>
    </div>
    <div class="alert" v-show="setadd">
      <div class="left"><img src="../../static/img/OK.png" alt=""></div>
      <div class="right">{{this.language.success[1]}}</div>
    </div>
    <div class="alert all" v-show="remcomment">
      <div class="left"><img src="../../static/img/X.png" alt=""></div>
      <div class="right">{{this.language.fail[0]}}</div>
    </div>
    ...
    <div class="notice_car" v-show="altcomment">
      ...
      <div class="bottom">
        <div class="car_text">
          <div class="show">{{this.language.car[0]}}<p v-for="(item) in this.dataAll" style="display: inline-block"><b style="font-weight: normal">{{item+'、'}}</b></p>{{i}}{{this.language.car[1]}}
          </div>
        </div>
        <div class="check">
          <div class="div_li">
  ...
  export default {
    ...
    data () {
      return {
        ...
        language: {},
        en: {
          top: ['Remove Monitoring', 'Notify Vehicles', 'Contact Baidu'],
          but: ['Search', 'Reset'],
          label: ['Vehicle ID', 'Vehicle Type', 'Version'],
          success: ['Remove Monitored Vehicles Successfully', 'Send Successfully'],
        ...
        cardata: '',
        addser: {
          addurl: 'http://localhost:3000/putDataIntoMonitor',
          comurl: 'http://localhost:3000/deleteMonitorDataById',
    ...
    methods: {
      getMonitorCarList () {
        this.$http.get('http://localhost:3000/getMonitorCarList?language=' + this.lang, {
          params: {
      ...
      cancel () {
        var that = this
        var car = this.dataAll.toString()
        if (car.length !== 0) {
          this.$http.post(this.addser.comurl, {id: car})
            .then((res) => {
              var that = this
